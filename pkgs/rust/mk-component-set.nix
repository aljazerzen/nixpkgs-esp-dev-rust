    ...
    stdenvNoCC.mkDerivation rec {
      ...
      postInstall = ''
        ...
        if [[ -e "$out/etc" ]]; then
          ...
          # Newer Rust releases install target-spec-json-schema.json in etc/.
          # Keep the directory in the sysroot so rustc can find the schema.
          rmdir "$out/etc" 2>/dev/null || true
        fi
      '';
